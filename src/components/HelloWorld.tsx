...
export default defineComponent((props, { emit }) => {
	...
	// 初始化
	const initQuery = () => {
		query.value = props.config.reduce((obj, config) => {
	...
	const filterFields = computed(() =>
		[...props.config].splice(0, isExpand.value ? props.config.length : oneLineMaxComponent - 1)
	)

	// 是否展开
	const isExpand = ref(props.config.length <= oneLineMaxComponent - 1)
	const span = computed(() =>
		isExpand.value ? (oneLineMaxComponent - (props.config.length % oneLineMaxComponent)) * 6 || 6 : 6
	)
	...
	const createElInput = (config: ISearchConfig) => {
		return (
			<a-input
				...
				{{
					prefix: () =>
						config.slot?.prefix?.() || <iconify class='text-base text-gray-400' icon='ep:search'></iconify>,
				...
				<a-col span={span.value}>
					<div class='flex-end'>
						{props.config.length > oneLineMaxComponent && (
							<a-button type='link' onClick={() => (isExpand.value = !isExpand.value)}>
